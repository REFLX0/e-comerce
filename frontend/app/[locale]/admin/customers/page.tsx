    ...
    <div className="p-4 sm:p-6 space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{total} clients enregistrés</p>
        </div>
      </div>

      <div className="relative max-w-md">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
        <input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Nom, email..." className="w-full rounded-xl border border-gray-200 bg-white py-2.5 pr-4 pl-10 text-sm outline-none focus:border-brand-accent transition-all" />
      </div>
          ...
          filtered.map((c: any) => (
            <Link key={c.id} href={localizedHref(`/admin/customers/${c.id}`)} className="block rounded-2xl border border-gray-100 bg-white p-4 shadow-sm hover:shadow transition-shadow">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="min-w-0 flex-1"><p className="text-sm font-semibold text-brand-primary truncate">{c.name ?? 'Utilisateur'}</p><p className="text-xs text-gray-400 truncate">{c.email}</p></div>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-1 text-gray-500"><ShoppingBag size={12} /> {c.ordersCount ?? 0} commandes</span>
                <span className="font-semibold text-brand-primary">{(c.ltv ?? 0).toLocaleString('fr-TN', { minimumFractionDigits: 2 })} TND</span>
              </div>
            </Link>
          ))
        )}
                  ...
                  <tr key={c.id} className="border-t border-gray-50 hover:bg-gray-50/50 transition-colors">
                    <td className="py-3 pl-4 pr-2">
                      <Link href={localizedHref(`/admin/customers/${c.id}`)} className="flex items-center gap-3">
                        ...
                        <div><p className="text-sm font-medium text-brand-primary">{c.name ?? 'Utilisateur'}</p><p className="text-xs text-gray-400">{c.email}</p></div>
                      </Link>
                    </td>
                    ...
                    <td className="px-2 py-3 text-xs text-gray-500">{c.createdAt ? new Date(c.createdAt).toLocaleDateString('fr-TN') : '—'}</td>
