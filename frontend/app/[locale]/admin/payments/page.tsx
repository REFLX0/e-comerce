...
import { CreditCard, Banknote, Clock, CheckCircle2, Search, ChevronDown } from 'lucide-react'
...
export default function AdminPaymentsPage() {
  ...
  const { data, isLoading } = useQuery<any>({
    ...
    queryFn: () => adminApi.getPayments({ page: 1, limit: 100 }),
  })
                ...
                filtered.map(tx => (
                  <tr key={tx.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-mono font-medium text-brand-primary">#{tx.id.slice(-8).toUpperCase()}</div>
                      <div className="text-xs text-gray-400">Cmd #{tx.orderId.slice(-8).toUpperCase()}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-gray-500 text-xs">
                      {new Date(tx.createdAt).toLocaleDateString('fr-TN', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </td>
                    <td className="px-6 py-4">
