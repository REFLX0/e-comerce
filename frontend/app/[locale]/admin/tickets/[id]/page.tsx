...
export default function TicketDetailPage() {
  ...
  if (!data) return <div className="p-6 text-center text-gray-400">Ticket introuvable</div>
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-brand-primary">Ticket {ticket.id ? `#${ticket.id.slice(-8).toUpperCase()}` : ''}</h1>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-4">
          <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="font-bold text-brand-primary">{ticket.type === 'RETURN' ? 'Retour produit' : 'Support client'}</p>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <h2 className="font-bold text-brand-primary mb-4">Détails</h2>
            <div className="space-y-3 text-sm">
              ...
              {ticket.createdAt && (
                <div className="flex items-center gap-2 text-gray-500">
                  <Calendar size={14} />
                  <span>{new Date(ticket.createdAt).toLocaleDateString('fr-TN')}</span>
                </div>
              )}
          ...
          {ticket.status !== 'RESOLVED' && (
            ...
            >
              ...
              {resolveMutation.isPending ? '...' : 'Marquer comme résolu'}
            </button>
          )}
        </div>
      </div>
