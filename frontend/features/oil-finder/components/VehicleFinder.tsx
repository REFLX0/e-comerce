...
export function VehicleFinder({ onClose, initialVehicleType }: VehicleFinderProps) {
  ...
  const loadEngines = (make: VehicleMake, model: VehicleModel) => {
    ...
    productsApi.getEngines(make.name, model.name)
      .then(data => { if (Array.isArray(data)) setEngines(data) })
  ...
  const triggerClass = (isOpen: boolean, isSelected: boolean, disabled: boolean) => {
    ...
    return 'border-white/10 bg-white/5 text-white/50 hover:border-white/20 hover:bg-white/8 cursor-pointer'
  }

  return (
    <div
      ref={containerRef}
      className="relative mx-auto w-full max-w-5xl overflow-visible rounded-2xl border border-white/10 bg-[#16254c] shadow-[0_24px_80px_rgba(22,37,76,0.45)]"
    >
      {/* ══════ HEADER ══════ */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-white/50 leading-none mt-0.5 hidden sm:block">
              Choisissez votre marque, modèle et motorisation pour afficher l&apos;huile homologuée
            </p>
          </div>
        </div>
        {selectedMake && (
      ...
      {/* ══════ 3 DROPDOWNS — row on desktop, stacked on mobile ══════ */}
      <div className={`p-5 sm:p-7 md:p-8 transition-[padding] duration-200 ${activeDropdown ? 'pb-72 md:pb-8' : 'pb-6 sm:pb-7 md:pb-8'}`}>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">

          {/* ─── 1. MARQUE ─── */}
          <div className="relative flex flex-col gap-2">
            <label className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-white/60">
            ...
            <button
              ...
              className={`w-full flex items-center justify-between gap-2 rounded-xl border px-4 py-3 text-sm font-semibold text-left transition-all outline-none ${triggerClass(activeDropdown === 'make', !!selectedMake, false)}`}
            >
              <span className="min-w-0 truncate">
                {selectedMake
                  ? <span className="font-black text-white">{selectedMake.name}</span>
                  : <span className="text-white/50">Sélectionner une marque…</span>
                }
              </span>
              <div className="flex shrink-0 items-center gap-1">
                {selectedMake && (
                  <span
                    ...
                    className="rounded-full p-1 text-white/30 transition hover:bg-white/10 hover:text-white/70"
                  >
                    <X size={12} />
                  </span>
                )}
            ...
            {/* DROPDOWN: MARQUE */}
            {activeDropdown === 'make' && (
              <div className="absolute left-0 right-0 top-[calc(100%+6px)] z-[200] rounded-2xl border border-slate-200/80 bg-white p-3.5 shadow-[0_20px_60px_rgba(0,0,0,0.3)] animate-in fade-in zoom-in-95 duration-150 md:min-w-[320px]">
                <div className="relative mb-3">
                  ...
                  <input
                    ...
                    placeholder="Rechercher (ex: Renault, Ford)…"
          ...
          {/* ─── 2. MODÈLE ─── */}
          <div className="relative flex flex-col gap-2">
            <label className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-white/50">
            ...
            <button
              ...
              className={`w-full flex items-center justify-between gap-2 rounded-xl border px-4 py-3 text-sm font-semibold text-left transition-all outline-none ${triggerClass(activeDropdown === 'model', !!selectedModel, !selectedMake || loading)}`}
            >
                ...
                {selectedModel ? (
                  <>
                    ...
                    {formatYearBadge(selectedModel.yearFrom, selectedModel.yearTo) && (
                      <span className="shrink-0 rounded-md bg-[#D4A76A]/20 border border-[#D4A76A]/30 px-1.5 py-0.5 text-[10px] font-bold text-[#D4A76A]">
                        {formatYearBadge(selectedModel.yearFrom, selectedModel.yearTo)}
                      </span>
                    )}
                  </>
                  ...
                  <span className={!selectedMake ? 'text-white/35' : 'text-white/50'}>
                    {!selectedMake ? "Sélectionnez d'abord la marque" : loading ? 'Chargement…' : 'Sélectionner un modèle…'}
                  </span>
                )}
              </div>
              <div className="flex shrink-0 items-center gap-1">
                {loading && <Loader2 size={13} className="animate-spin text-[#D4A76A]" />}
            ...
            {activeDropdown === 'model' && selectedMake && (
              <div className="absolute left-0 right-0 top-[calc(100%+6px)] z-[200] rounded-2xl border border-slate-200/80 bg-white p-3.5 shadow-[0_20px_60px_rgba(0,0,0,0.3)] animate-in fade-in zoom-in-95 duration-150 md:min-w-[340px]">
                <div className="relative mb-3">
                  <Search size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
                  <input
                    ref={modelInputRef}
          ...
          <div className="relative flex flex-col gap-2">
            ...
            <button
              ...
              className={`w-full flex items-center justify-between gap-2 rounded-xl border px-4 py-3 text-sm font-semibold text-left transition-all outline-none ${triggerClass(activeDropdown === 'engine', !!selectedEngine, !selectedModel || loading)}`}
            >
              <div className="flex min-w-0 flex-1 items-center gap-2 truncate">
                {selectedEngine ? (
                  <>
            ...
            {activeDropdown === 'engine' && selectedModel && (
              <div className="absolute left-0 right-0 md:left-auto md:right-0 top-[calc(100%+6px)] z-[200] rounded-2xl border border-slate-200/80 bg-white p-3.5 shadow-[0_20px_60px_rgba(0,0,0,0.3)] animate-in fade-in zoom-in-95 duration-150 md:w-[440px]">
                {/* Fuel Filter Pills */}
                ...
                <div className="relative mb-3">
                  ...
                  <input
                    ...
                    placeholder="Filtrer (ex: 1.0, 125ch, EcoBoost)…"
                    className="w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-8 py-2.5 text-xs font-semibold text-slate-900 outline-none focus:border-[#16254c] focus:ring-2 focus:ring-[#16254c]/15 transition"
                  />
                  {engineSearch && (
        ...
        {/* ══════ ASSURANCE BADGES (when vehicle not yet selected) ══════ */}
        {(!selectedMake || !selectedModel) && (
          <div className="mt-6 pt-4 border-t border-white/10 flex flex-wrap items-center justify-between gap-3 text-[11px] text-white/50">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>Filtrage direct par motorisation & cylindrée</span>
            </div>
            <div className="hidden md:flex items-center gap-2">
              <Gauge size={14} className="text-[#D4A76A] shrink-0" />
              <span>Normes officielles ACEA & API</span>
            </div>
          </div>
        )}

        {/* ══════ CONFIRMED VEHICLE + CTA ══════ */}
        {selectedMake && selectedModel && (
          <div className="mt-6 flex flex-col gap-3 rounded-xl border border-white/10 bg-white/5 p-4 sm:flex-row sm:items-center sm:justify-between animate-in fade-in duration-300">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-sm font-black text-white flex flex-wrap items-center gap-1.5">
                  ...
                  {selectedEngine && (
                    <span className="text-white/50 font-semibold">· {selectedEngine.engineCode}</span>
                  )}
                </p>
                {activeSpec && (
