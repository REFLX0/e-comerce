...
export class VehiclesService {
  ...
  async getCompatibleProducts(
    ...
    if (compatibilities.length === 0 && model && engineCode) {
    ...
    if (compatibilities.length > 0) {
      return this.rankAndHideSourcing(compatibilities.map((item) => item.product));
    }
   ...
   * Returns matching products from the store catalogue.
   */
  async getCompatiblePage(
    ...
    if (compatibilities.length > 0) {
      ...
      // Find products in store catalogue mentioning make or model name
      const searchTerms = [make?.name || makeSlug, model?.name || modelSlug].filter(Boolean);
