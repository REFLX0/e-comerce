...
export function generateDeliveryNotePDF(order: InvoiceOrder) {
  ...
  function addHeaderFooter() {
    ...
    doc.fontSize(8).fillColor(grayColor)
      .text('specpart.tn — Tunis', leftMargin, bottomY, { align: 'center', width: pageWidth - 80 });
  }
  ...
  doc.moveTo(leftMargin, 155).lineTo(pageWidth - rightMargin, 155).strokeColor('#e0e0e0').stroke();
  ...
  doc.font('Helvetica').fillColor(grayColor)
    .text(new Date(order.createdAt).toLocaleDateString('fr-TN'), leftMargin + 90, orderY + 18);

  doc.font('Helvetica-Bold').fillColor(primaryColor)
    .text(`Statut:`, leftMargin, orderY + 36);
  doc.font('Helvetica').fillColor(grayColor)
    .text(order.status === 'DELIVERED' ? 'Livrée' : order.status === 'SHIPPED' ? 'Expédiée' : order.status === 'CONFIRMED' ? 'Confirmée' : 'En attente', leftMargin + 90, orderY + 36);
  ...
  doc.text(`${order.shipCity || '-'}, ${order.shipWilaya || '-'}`, clientX + 50, orderY + 36);

  // ── SEPARATOR ──
  const tableY = 240;
  doc.moveTo(leftMargin, tableY - 5).lineTo(pageWidth - rightMargin, tableY - 5).strokeColor('#e0e0e0').stroke();
  ...
  order.items.forEach((item, idx) => {
    ...
    doc.text(item.variant?.volume || '-', leftMargin + 230, currentY, { width: 60 });
    doc.text(String(item.quantity), leftMargin + 290, currentY, { width: 40 });
    doc.text(`${item.unitPrice.toFixed(2)} TND`, leftMargin + 330, currentY, { width: 60 });
    doc.text(`${(item.unitPrice * item.quantity).toFixed(2)} TND`, leftMargin + 410, currentY, { width: 80 });
  ...
  // ── TOTAL ──
  currentY += 10;
  doc.moveTo(leftMargin + 250, currentY).lineTo(pageWidth - rightMargin, currentY).strokeColor('#e0e0e0').stroke();
  currentY += 12;

  doc.font('Helvetica-Bold').fontSize(11).fillColor(primaryColor);
  doc.text('Total TTC:', leftMargin + 300, currentY);
  doc.fillColor(accentColor).text(`${order.totalAmount.toFixed(2)} TND`, leftMargin + 410, currentY);
  ...
  if (order.notes) {
    ...
    doc.text(`Note: ${order.notes}`, leftMargin, currentY, { width: pageWidth - 80 });
  }
