/**
 * One-off import: "Filtre habitacle" (cabin filter) catalog from a CSV (same
 * layout as the other filter categories) + local watermarked image folder.
 *
 * Mixed sourcing: MISFAT rows were scraped from bestoil.tn (not tomobile.store)
 * per explicit user instruction, everything else from tomobile.store as usual —
 * the CSV already has this baked in, this script doesn't need to know the
 * difference. Same approach otherwise: compatibleVehiclesNote /
 * technicalCharacteristics / oemReferences written directly at creation time,
 * short factual 3-line description generated up front instead of importing
 * the long marketing-style "Description détaillée" column.
 *
 * Usage (inside the backend container):
 *   npx tsx scripts/import-cabin-filters.ts            # dry-run report
 *   npx tsx scripts/import-cabin-filters.ts --apply     # write changes + upload images
 */
...
const CSV_PATH = path.join(DATA_DIR, 'tomobile_filtres_habitacle.csv');
const CATEGORY_ID = 'cmtnthkmm000qnpcyyvb395yl'; // Filtre habitacle
...
// Unlike "filtre à air/carburant/huile", the established term here has no
// preposition — every source title says "Filtre Habitacle", not "Filtre à habitacle".
const FILTER_PHRASE = 'filtre habitacle';
...
function generateDescription(brand: string, sku: string, makes: string[], filterType: string | null, vehicleCount: number, oemCount: number): string {
  ...
  const line1Variants = [
    `${brandPart} — ${FILTER_PHRASE}${hasType ? `, ${typeLabel}` : ''}.`,
    `${FILTER_PHRASE.charAt(0).toUpperCase() + FILTER_PHRASE.slice(1)} ${brandPart}${hasType ? ` (${typeLabel})` : ''}.`,
    `${brandPart} : ${FILTER_PHRASE}${hasType ? ` de type ${typeLabel}` : ''}.`,
    `${FILTER_PHRASE.charAt(0).toUpperCase() + FILTER_PHRASE.slice(1)} d'origine ${brandPart}.`,
  ];
